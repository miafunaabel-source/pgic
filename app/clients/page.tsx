...
import { Search, Phone, Mail, ChevronRight, Users, UserCheck, UserX, CalendarPlus, Calendar } from "lucide-react";
import type { Client } from "@/lib/data";
import { formatDate, formatCurrency, cn } from "@/lib/utils";
import Modal from "@/components/ui/Modal";
import { useToast } from "@/contexts/ToastContext";

const HEURES_RDV = ["08:00","08:30","09:00","09:30","10:00","10:30","11:00","11:30","14:00","14:30","15:00","15:30","16:00","16:30","17:00"];
const TYPES_RDV = [
  { value: "decouverte", label: "Découverte client" },
  { value: "essai", label: "Essai véhicule" },
  { value: "devis", label: "Présentation devis" },
  { value: "livraison", label: "Livraison véhicule" },
...
export default function ClientsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setShowModal(false);
    setForm({ prenom: "", nom: "", email: "", telephone: "", ville: "" });
    setWithRdv(false);
    if (withRdv) {
      const typeLabel = TYPES_RDV.find(t => t.value === rdvForm.type)?.label ?? rdvForm.type;
      toast("success", `Prospect ${form.prenom} ${form.nom} ajouté — RDV ${typeLabel} le ${new Date(rdvForm.date).toLocaleDateString("fr-FR")} à ${rdvForm.heure}`);
    } else {
      toast("success", `Prospect ${form.prenom} ${form.nom} ajouté`);
    }
  }
  ...
  return (
    <div>
      ...
      {showModal && (
        <Modal title="Nouveau client" onClose={() => { setShowModal(false); setWithRdv(false); }}>
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="border-t border-slate-100 pt-3">
              ...
              >
                ...
                <span className={cn("text-xs font-medium px-2 py-0.5 rounded-full", withRdv ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-500")}>
                  {withRdv ? "Activé" : "Optionnel"}
                </span>
              </button>

              {withRdv && (
                <div className="mt-3 space-y-3 bg-blue-50/50 border border-blue-100 rounded-xl p-3">
                  <div className="flex items-center gap-2 text-xs font-semibold text-blue-700 mb-1">
                    <Calendar size={13} /> Détails du rendez-vous
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      >
                        {HEURES_RDV.map(h => <option key={h}>{h}</option>)}
                      </select>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-700 mb-1">Type de RDV</label>
                    <select
                      value={rdvForm.type}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Intérêt client, véhicule souhaité…"
                      className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                    />
                  </div>
                </div>
            ...
            <div className="flex gap-3 pt-1">
              <button type="button" onClick={() => { setShowModal(false); setWithRdv(false); }} className="flex-1 btn-secondary">Annuler</button>
              <button type="submit" className="flex-1 btn-primary">
                {withRdv ? "Créer + planifier RDV" : "Créer le prospect"}
              </button>
            </div>
          </form>
        </Modal>
        ...
        <div className="flex gap-1 bg-white border border-slate-200 rounded-lg p-1">
          {(["tous", "client", "prospect", "inactif"] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={cn("px-3 py-1 rounded text-sm font-medium capitalize transition-colors", filter === f ? "bg-blue-600 text-white" : "text-slate-600 hover:bg-slate-50")}>
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {filtered.map(client => (
              <tr key={client.id} className="border-b border-slate-50 hover:bg-slate-50 cursor-pointer transition-colors" onClick={() => setSelected(client)}>
        ...
        {!loading && clients.length === 0 && !search && (
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="font-medium text-slate-500">Aucun client pour l'instant</p>
            <p className="text-sm mt-1">Ajoutez votre premier client avec le bouton ci-dessus</p>
          </div>
        )}
      </div>
    </div>
